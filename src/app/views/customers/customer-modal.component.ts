      ...
      <div class="custom-modal-backdrop" (click)="close()">
        <div class="custom-modal-content" (click)="$event.stopPropagation()">
          <div class="custom-modal-header">
            <h5 class="fw-bold">{{ isEditMode ? 'Edit Customer Details' : 'Create New Customer' }}</h5>
            <button type="button" class="btn-close" (click)="close()" aria-label="Close"></button>
          </div>
          <div class="custom-modal-body">
            <form cForm [formGroup]="customerForm">
              <div class="mb-3">
                <label class="small fw-semibold text-secondary mb-1">Customer Name *</label>
                <input formControlName="name" cFormControl placeholder="e.g. John Doe / Importadores Inc" />
                @if (customerForm.get('name')?.touched && customerForm.get('name')?.invalid) {
                  <div class="text-danger small mt-1">Name is required (Max 150 characters).</div>
                }
              </div>

              <div class="mb-3">
                <label class="small fw-semibold text-secondary mb-1">Identification Number</label>
                <input formControlName="identification" cFormControl placeholder="e.g. RNC / Passport ID" />
              </div>

              <c-row>
                <c-col md="6" class="mb-3">
                  <label class="small fw-semibold text-secondary mb-1">Email</label>
                  <input formControlName="email" cFormControl placeholder="e.g. customer@web.com" />
                ...
                <c-col md="6" class="mb-3">
                  <label class="small fw-semibold text-secondary mb-1">Phone Number</label>
                  <input formControlName="phone" cFormControl placeholder="e.g. 555-0199" />
                </c-col>
              </c-row>
            </form>
          </div>
